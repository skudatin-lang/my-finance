import{$,fmt,state,MONTHS,getMOps,isPlanned,planSpent,today,detectAnomalies}from'./core.js';
...
// FIX: uses shared detectAnomalies from core.js (2*std, 6 months — same as dashboard)
function renderAnomalies(){
  const el=$('analytics-anomalies');if(!el)return;
  const factOps=getMOps(0).filter(o=>!isPlanned(o.type));
...
export function exportAllCSV(){
  ...
  a.download=`finance-all-${today()}.csv`;a.click();
}
