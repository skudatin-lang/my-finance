import{$,fmt,state,sched,today,isPlanned}from'./core.js';
    ...
    return`<div style="background:${bg};border:1.5px solid ${border};border-radius:10px;padding:12px 14px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div style="flex:1">
          ...
          <div style="font-size:11px;color:var(--text2);margin-top:2px">${t.type==='income'?'Доход':'Расход'} · ${t.category} · ${t.wallet?wName(t.wallet):''}</div>
...
function wName(id){
  const w=state.D.wallets.find(w=>w.id===id);return w?w.name:'';
}
...
function applySingle(t){
  ...
  if(window['render'+capitalize(cur)])window['render'+capitalize(cur)]();
}

function applyBulk(t){
  // Open bulk modal pre-filled
  openBulkModal(t);
}
