...
// БАГ-ФИX: на мобиле последние шаги ищут элементы в скрытом topbar-row2
// Решение: на мобиле ВСЕГДА показываем карточку внизу по центру (position: center)
import { $ } from './core.js';
...
// Проверяем, видим ли элемент на экране (не скрыт через display:none или visibility:hidden)
...
function _positionCard(el, position) {
  ...
  // ── МОБИЛЬ: ВСЕГДА внизу по центру ─────────────────────────────────
  // Независимо от шага и наличия targetId — на мобиле карточка фиксирована внизу.
  // Это предотвращает вылет за экран когда targetEl скрыт (topbar-row2 hidden).
  if (isMobile) {
    ...
    card.style.top = 'auto';
    // Учитываем bottom-nav (60px) + небольшой отступ
...
function _renderStep(idx) {
  ...
  // Выполняем экшен (переключение экрана)
  if (step.action) step.action();

  // Ищем целевой элемент — но только если он реально виден
  const rawTarget = step.targetId ? document.getElementById(step.targetId) : null;
  // На мобиле: если элемент скрыт (в hidden topbar-row2) — не используем spotlight
  const targetEl = (rawTarget && _isElementVisible(rawTarget)) ? rawTarget : null;
