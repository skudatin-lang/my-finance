import{$,fmt,fmtS,state,MONTHS,planById,catPlanId,isPlanned,planSpent,wName}from'./core.js';

export function renderDDS(){
  if(!state.D)return;

  // ❗ ВАЖНО: теперь берём ВСЕ операции, а не только месяц
  ...
  sorted.forEach(o=>{
    ...
    const cat=o.type==='transfer'
      ?`Перевод \u2192 ${wName(o.walletTo)}`
      :(o.category||'—');
