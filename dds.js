...
export function renderDDS(){
  ...
  if(!ops.length){table.innerHTML='<tr><td colspan="4" style="padding:20px;text-align:center;color:var(--text2)">Нет операций</td></tr>';return;}
  ...
  sorted.forEach(o=>{
    ...
    html+=`<tr><td>${o.date?o.date.split('-').reverse().join('.'):'—'}</td><td>${cat}${plbl}${o.note?'<br><span style="font-size:10px;color:var(--text2)">'+o.note+'</span>':''}</td><td>${wName(o.wallet||'')}</td><td class="${cls}" style="text-align:right">${pfx} ${fmt(o.amount)}</td></tr>`;
  });
