/**
 * voice.js — Yandex SpeechKit STT + GPT
 *
 * КЛЮЧЕВОЕ РЕШЕНИЕ: ВСЕГДА конвертируем аудио в PCM 16-bit mono
 * через AudioContext перед отправкой. Это работает во ВСЕХ браузерах
 * (Safari, Chrome, Firefox) и исключает проблемы с форматами контейнеров
 * (ogg header not found, webm-opus и т.д.)
 */
...
// ── Конвертация любого аудио → PCM 16-bit 16kHz mono ─────────────────────
// Работает с любым форматом: webm, ogg, mp4 — всё через AudioContext
const PCM_RATE = 16000;

async function _toPCM(blob) {
  const arrayBuf = await blob.arrayBuffer();
  const AudioCtx = window.AudioContext || window.webkitAudioContext;
  if (!AudioCtx) throw new Error('AudioContext не поддерживается браузером');

  const ctx = new AudioCtx({ sampleRate: PCM_RATE });
  let decoded;
  try {
    decoded = await ctx.decodeAudioData(arrayBuf);
  } catch (e) {
    await ctx.close();
    throw new Error('Не удалось декодировать аудио: ' + e.message);
  }
  await ctx.close();
  ...
  // Ресэмплируем если реальный rate отличается от 16000
  if (decoded.sampleRate !== PCM_RATE) {
    const ratio  = decoded.sampleRate / PCM_RATE;
    const newLen = Math.ceil(samples.length / ratio);
    const rs     = new Float32Array(newLen);
    for (let i = 0; i < newLen; i++) {
      rs[i] = samples[Math.min(Math.floor(i * ratio), samples.length - 1)];
    }
    samples = rs;
  }
...
// Берём первый поддерживаемый — неважно какой, всё равно конвертируем в PCM
function _pickMime() {
  const candidates = [
...
export async function startRecording(onResult, onError, onStateChange) {
  ...
  if (!mime) {
    onError && onError('Браузер не поддерживает запись аудио.\nИспользуйте Chrome, Firefox или Safari 14.1+.');
    return;
  }
  ...
  try {
    ...
    onError && onError(
      e.name === 'NotAllowedError'
        ? 'Нет доступа к микрофону.\nРазрешите в настройках браузера (значок 🔒 в адресной строке).'
  ...
  recorder.onstop = async () => {
    stream.getTracks().forEach(t => t.stop()); // сразу освобождаем
    onStateChange && onStateChange(false);

    if (!_audioChunks.length) {
      onError && onError('Нет аудио. Попробуйте ещё раз.');
      return;
    }

    try {
      // ВСЕГДА конвертируем в PCM — работает в Safari, Chrome, Firefox
      _showToast('⏳ Обработка аудио...');
      const rawBlob  = new Blob(_audioChunks, { type: mime });
      const pcmBuf   = await _toPCM(rawBlob);
      const text     = await _sendPCM(pcmBuf);
...
async function _sendPCM(pcmBuffer) {
  if (!pcmBuffer || pcmBuffer.byteLength < 500) {
    console.warn('[STT] PCM buffer too small:', pcmBuffer?.byteLength);
    return null;
  }
  ...
  console.log(`[STT] → ${url} lpcm ${pcmBuffer.byteLength}b @ ${PCM_RATE}Hz`);

  try {
    const resp = await fetch(url, { method: 'POST', headers, body: pcmBuffer });
    const data = await resp.json();
    console.log(`[STT] ← ${resp.status} result="${data.result||''}" err="${data.error_message||''}"`);

    if (!resp.ok) {
      const msg = data.error_message || data.error || data.message || JSON.stringify(data).slice(0, 200);
      if (resp.status === 401) _showToast('⚠ Ошибка авторизации. Проверьте YANDEX_API_KEY в воркере.');
    ...
    if (!result) {
      console.warn('[STT] empty result:', JSON.stringify(data));
      return null;
    }
    return result;
...
export async function parseIntent(spokenText){
  ...
  const sys=`Ты ассистент по личным финансам. Пользователь надиктовал команду.
Определи намерение и верни ТОЛЬКО JSON (без markdown, без пояснений).
Намерения: add_expense(amount,category,wallet,note), add_income(amount,category,wallet,note), add_transfer(amount,from_wallet,to_wallet), add_shopping(items:[{name,qty,price}]), check_balance(wallet?), add_goal(name,target_amount,deadline?), add_category(name,type), unknown(raw_text).
Категории: ${cats.join(', ')}. Кошельки: ${wallets.join(', ')}. Дата: ${today()}.`;
  ...
  try{
    ...
    if(!resp.ok)return _fallback(spokenText);
...
function _fallback(text){
  ...
  if(t.match(/купи|список|магазин/))return{intent:'add_shopping',items:[{name:text.replace(/\d+/g,'').trim()||text,qty:1,price:0}]};
...
export function handleVoiceIntent(intent,onConfirm){
  ...
  switch(intent.intent){
    case'add_expense':case'add_income':
      body=`<b>${intent.amount?fmt(intent.amount):'(сумма?)'}</b>`+(intent.category?` · ${intent.category}`:'')+( intent.note?`<br><span style="color:var(--text2);font-size:11px">${intent.note}</span>`:'');break;
    case'add_shopping':
      body=(intent.items||[]).map(i=>`• <b>${i.name}</b>${i.qty>1?' × '+i.qty:''}${i.price?' — '+fmt(i.price):''}`).join('<br>')||'(нет позиций)';break;
    case'add_transfer':
    ...
    case'check_balance':
      if(state.D){const w=intent.wallet?state.D.wallets.find(w=>w.name.toLowerCase().includes(intent.wallet.toLowerCase())):null;body=w?`${w.name}: <b>${fmt(w.balance)}</b>`:`Общий: <b>${fmt(state.D.wallets.reduce((s,w)=>s+w.balance,0))}</b>`;}break;
    case'add_goal':
...
export function executeIntent(intent){
  ...
  switch(intent.intent){
    ...
    case'add_transfer':{
      ...
      state.D.operations.push(op);sched();_showToast(`✓ Перевод ${fmt(intent.amount)} выполнен`);
      window._refreshCurrentScreen&&window._refreshCurrentScreen();break;
    }
    case'add_goal':{
...
export function createSmartVoiceButton(){
  ...
  let active=false;
  btn.onclick=async()=>{
    if(!isVoiceConfigured()){alert('Голосовой ввод не настроен.\n\nПерейдите: Администратор → введите URL воркера → Сохранить.');return;}
    if(active){stopRecording();return;}
    await startRecording(
      async text=>{active=false;btn.textContent='🎤';btn.style.background='';btn.style.transform='';_showToast('🔍 «'+text+'» — анализирую...');const intent=await parseIntent(text);handleVoiceIntent(intent,executeIntent);},
