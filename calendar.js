...
// FIX: removed duplicate wName/fmtD local definitions — use imported ones from core.js
window.showCalSummary=function(type){
  ...
  if(type==='fact-inc'){ops=allM.filter(o=>o.type==='income');title='Фактические доходы';colorCls='pos';}
  else if(type==='fact-exp'){ops=allM.filter(o=>o.type==='expense');title='Фактические расходы';colorCls='neg';}
  else if(type==='plan-inc'){ops=allM.filter(o=>o.type==='planned_income');title='Плановые доходы';colorCls='blue';}
  ...
  if(!ops.length){el.innerHTML=`<div style="color:var(--text2);font-size:12px;padding:4px 0">Нет операций</div>`;return;}
  ...
  // FIX: use imported wName and fmtD from core.js (no local redefinition)
