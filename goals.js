...
function fillGoalWallets(selectedId){
  $('goal-wallet').innerHTML=state.D.wallets.map(w=>`<option value="${w.id}"${w.id===selectedId?' selected':''}>${w.name}</option>`).join('');
}
...
// FIX: fill wallet select before setting value
